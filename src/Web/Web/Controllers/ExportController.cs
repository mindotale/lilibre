...
public class ExportController : Controller
{
    ...
    public FileStreamResult ToCSV(IQueryable query, string fileName = null)
    {
        ...
        var result = new FileStreamResult(
            new MemoryStream(
                Encoding.Default.GetBytes($"{string.Join(",", columns.Select(c => c.Key))}{Environment.NewLine}{sb}")),
            "text/csv");
        ...
        using (var document = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook))
        {
            ...
            foreach (var item in query)
            {
                ...
                foreach (var column in columns)
                {
                    ...
                    var underlyingType =
                        column.Value.IsGenericType && column.Value.GetGenericTypeDefinition() == typeof(Nullable<>)
